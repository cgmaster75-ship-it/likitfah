...
function parseSanookLotto(html) {
    ...
    // 1. Remove all currency/monetary values containing commas (e.g., 6,000,000 บาท, 4,000 บาท)
    // This is extremely safe because lottery numbers never contain commas!
    cleanText = cleanText.replace(/\b\d{1,3}(?:,\d{3})+\s*บาท/g, '');
    
    // 2. Remove "รางวัลละ" or "รางวัลๆละ" details
    cleanText = cleanText.replace(/รางวัลละ\s*บาท/g, '');
    cleanText = cleanText.replace(/รางวัลๆละ\s*บาท/g, '');
    
    // 3. Remove "มี X รางวัล" or "X รางวัล" strictly on the same line (using horizontal space [ \t]*)
    cleanText = cleanText.replace(/มี\s*\d+\s*รางวัล/g, '');
    cleanText = cleanText.replace(/\b\d+[ \t]+รางวัล/g, '');
